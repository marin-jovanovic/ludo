...
export default {
  data() {
    return {
      ...
      types: [
        { type: "1st", colour: "#99c1f1" },
        { type: "2nd", colour: "#f9f06b" },
        { type: "3rd", colour: "#77767b" },
        { type: "4th", colour: "#722661" },
        { type: "quit", colour: "#000000" },
      ],
  ...
  methods: {
    setColumn(columnCount) {
      ...
      this.updateColumnChange();
    },
    oneClick(type, a, b) {
      for (let r = 0; r < this.types.length; r++) {
        if (type == this.types[r].type) {
          this.selected = this.types[r];
        }
      }
      // this.selected = typ
