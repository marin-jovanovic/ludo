...
import BaseMapOverlay from "./BaseMapOverlay.vue";
...
export default {
  ...
  methods: {
    ...
    async selectRow(portfolioPayload) {
      ...
      let r = await apiLocation.getAllLocationsInPortfolio(
        portfolioPayload.oldName
      );
      if (r["payload"]["status"]) {
  ...
  components: { TheLocationTable, BaseMapOverlay, BaseNotification },
};
</script>
